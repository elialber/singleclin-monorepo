...
class TokenRefreshService {
  ...
  void _onAuthStateChanged(UserEntity? user) {
    ...
    if (user != null) {
      _startTokenRefresh();
      if (kDebugMode) {
        print('🔄 TokenRefreshService: User authenticated, starting token refresh');
      }
    } else {
      _stopTokenRefresh();
      if (kDebugMode) {
        print('🔄 TokenRefreshService: User signed out, stopping token refresh');
      }
    }
  }
  ...
  void _startTokenRefresh() {
    ...
    if (kDebugMode) {
      print('🔄 TokenRefreshService: Token refresh timer started (${_refreshInterval.inMinutes} min intervals)');
    }
  }
  ...
  Future<String?> _refreshToken() async {
    try {
      if (_currentUser == null) {
        if (kDebugMode) {
          print('⚠️ TokenRefreshService: No authenticated user, skipping token refresh');
        }
        return null;
      }
      ...
      if (user == null) {
        if (kDebugMode) {
          print('⚠️ TokenRefreshService: Firebase user is null, stopping token refresh');
        }
        _stopTokenRefresh();
        return null;
      }
  ...
  void resume() {
    if (!_isActive || _currentUser == null) return;
