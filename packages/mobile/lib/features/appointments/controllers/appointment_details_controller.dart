...
class AppointmentDetailsController extends GetxController {
  ...
  Future<void> loadAppointmentDetails() async {
    ...
    try {
      ...
      final updatedAppointment = _appointment.value!.copyWith(
        ...
        preInstructions: _appointment.value!.preInstructions ?? 
            'Chegue 15 minutos antes do horário marcado. Evite maquiagem no dia do procedimento.',
        postInstructions: _appointment.value!.postInstructions ??
  ...
  Future<void> cancelAppointment() async {
    final result = await Get.toNamed('/appointments/cancel', 
  ...
  Future<void> rescheduleAppointment() async {
    final result = await Get.toNamed('/discovery/booking', arguments: {
      'reschedule': true,
      'appointment': _appointment.value,
    });
  ...
  Future<void> rateAppointment() async {
    final result = await Get.toNamed('/appointments/rate', 
        arguments: _appointment.value);
    ...
    switch (appointment.status) {
      ...
      case AppointmentStatus.confirmed:
        ...
        if (DateTime.now().isBefore(appointmentDateTime)) {
          events.add(TimelineEvent(
            ...
            description: '${appointment.serviceName} - ${appointment.formattedDate} às ${appointment.formattedTime}',
      ...
      case AppointmentStatus.cancelled:
        events.add(TimelineEvent(
          ...
          description: appointment.cancellationReason ?? 'Agendamento cancelado',
